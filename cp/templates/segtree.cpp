...
struct SegTree {
	...
	int find_kth(ll k) {
    return find_kth_util(1, 0, n - 1, k);	
	}

	int find_kth_util(int index, int start, int end, ll k) {
	if(k>tree[index]) return -1; 	// if k values are not present
    if (start == end)
        return start;

    int mid = (start + end) >> 1;

    if (tree[index * 2].val >= k)
        return find_kth_util(index * 2, start, mid, k);
    else
        return find_kth_util(index * 2 + 1, mid + 1, end, k - tree[index * 2].val);
}
